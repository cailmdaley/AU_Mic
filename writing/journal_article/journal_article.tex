...
The M3IVe star AU Mic presents a particularly favorable target for such observations due to its proximity (\SI{9.91 \pm 0.10}{pc}; \citealp{vanleeuwen07}) and edge-on inclination. 
...
\S \ref{section: results} quotes the basic observational results and compares the millimeter morphology of the disk with the morphology at shorter wavelengths.  
...
Consequently, the most accurate way to isolate the disk flux from the stellar contribution is through parametric modeling (see \S \ref{section: analysis}) where the two components can be specified separately; our modeling yields a disk flux of \SI{4.8 \pm 0.2}{mJy}.
...
It seems like likely that this variability is distinct from the \SI{6}{minute} flare spanning two orders of magnitude that occurred during the June observations, but it is not entirely impossible that the small-scale variation we see is the dying song of a flare.
\cite{cox85} detect small-scale ($\sim 50\%$) variations over a two-week period between \SIrange{2}{20}{cm} at the VLA, possibly due to `several independent mini-flares' or rotational modulation of the star.
