...
\begin{table}	
  ...
  \begin{tabular}{lr}
    \toprule
    Time (UTC) & Point-source Flux (\si{mJy}) \\
    \midrule
...
Using the \texttt{MIRIAD} task \texttt{cgcurs} \citep{sault95}, we measure an integrated flux of \SI{4.97 \pm 0.08}{\milli Jy} enclosed within the $3\sigma$ contours of the naturally weighted image.  
...
We set a $3 \sigma$ upper limit of \SI{0.07}{Jy.km.s^{-1}} on the CO~$\mathrm{J}=2-1$ integrated flux, obtained by integrating the flux in an $\SI{8}{\arcsecond} \times \SI{8}{\arcsecond}$ box around the star between $V_{LSRK}$ velocities of $-10$ and \SI{10}{km.s^{-1}}.
...
\citep{gaia_DR2}; the observed \SI{1.3}{mm} stellar flux $F_\star$ is left as a free parameter for each observation date.
...
Priors placed on the stellar flux and disk inner radius, width, power law exponent, and aspect ratio ensured that these parameters were always greater than zero.
...
The statistical weights are likely not responsible, due to our recalculation of the weights according to the observed variance within each grid point in the $u-v$ plane; these were checked to ensure that the distribution was indeed Gaussian.
...
The smallest naturally weighted beam FHWM from the three individual observations is \SI{0.30}{\arcsecond} ($\sim$\SI{3}{au}; Table \ref{tab:observations}), while the spatial scale traced by the longest baseline is \SI{0.22}{\arcsecond} ($\sim$\SI{2.2}{au}).
...
However such an analysis would be subject to assumptions about the grain size distribution and is beyond the scope of this vertical structure-oriented work.
...
\afterpage{%    % defer execution until the next page break occurs anyway
  ...
  \begin{table}[h!] % not t or b or p
    ...
    \begin{tabular}{lcccl}
      ...
      \midrule
      1a & \cite{schuppler17} & $ < \SI{70}{\mu m}$  & \SI{2}{au} & Collisional modeling \\
      1b & --- & \SI{0.445}{\mu m}--\SI{1.3}{mm}     & \SI{0.6}{au} & Collisional modeling \\
      1b & --- & $> \SI{100}{\mu m}$                 & \SI{0.2}{au} & Collisional modeling \\
      2a & \cite{metchev05}   & \SI{1.63}{\mu m}     & \SI{2}{au} & Apparent FWHM \\
