...
%EIC: Why is "inferred" lower case? Looks weird. I made upper case.
...
%EIC: removed typo dash in Berkeley address
...
\begin{abstract}
The vertical distribution of dust in debris disks is particularly sensitive to the total mass of planetary bodies stirring the disk. 
%EIC: emphasizing Uranus and Neptune analogs is misleading since the main
%      message of the paper is to constrain bodies much smaller in mass.
%      As a quick solution, I omitted the phrase.
%, and is therefore well suited for constraining the prevalence of otherwise unobservable Uranus and Neptune analogs. 
...
The large grains that dominate the emission at millimeter wavelengths are much less 
%EIC: For AU Mic, an M dwarf, stellar radiation is not significant compared to
%      stellar winds. It is not just that wind pressure is an add-on effect; it is that it
%      completely overwhelms radiation pressure in the case of AU Mic.
%      So at a minimum, when it concerns AU Mic, winds
%      should be mentioned at the same time radiation pressure is mentioned.
%      I did this below.
sensitive to the effects of stellar radiation or stellar winds, 
and therefore trace the underlying mass distribution more directly. 
...
Comparing the observed aspect ratio to a theoretical model of steady-state size-dependent velocity distributions in the collisional cascade,
%EIC: I think the following statement (which I have commented out) is not correct.
%       It implies that a *single* body, of mass 1.0e-4 M_earth (radius 410 km),
%       can stir the entire belt (assuming no damping). Even with no damping,
%       a single body takes time to stir up the local belt; the smaller the body,
%       the longer the time, and I suspect a single 410 km object would take much
%       longer than the age of the system (23 Myr) to stir up the disk.
%       The theoretical considerations of section 5.6 don't actually say a single
%       body can do this; they just say that the bodies (plural) have to be at least
%       410 km in size. I re-worded the rest of the abstract accordingly.
%we find that the total mass of bodies stirring the disk must fall between $\sim \SI{1.0e-4}{M_\earth}$ and $\SI{1.5}{M_\earth}$.  
...
%EIC: Not all planets form during the gas-rich phase. We know from Kepler
%       and ground-based surveys that most planets in the universe are not
%       gas giants, but rather super-Earths with low (<~ 10%) gas content.
%       The disks that spawn super-Earths may have pretty low gas-to-solid
%       contents, down to 1e-6 
%       (see Lee, Chiang, & Ferguson 2018, paper 4 of our series).
%       I wouldn't call these "rich in gas".
%
%       Recall also that terrestrial planets in our Solar System
%       form on timescales of ~1e8 yr (we know this from radionuclide data),
%       when little-to-no gas is left.
%
%       Debris disks are often thought of as the "aftermath" of planet formation,
%       but this is not a completely accurate characterization.
%       While big bodies must have formed prior to the debris disk phase
%       (to generate the cascade),
%       more big bodies may also be forming *during* the debris disk phase.
%
%       Simplest fix is to delete the phrase, "rich in gas", which is what I did.
...
%EIC: there is a difference between "because of" and "due to", although
%       these days the old rules are giving way to interchangeable use. You can
%       google the debate. Anyway, I followed the old rule which
%       everyone agrees is acceptable.
because of
%due to 
...
%EIC: I disagree with the characterization of "promising".
%       How a planet --- in particular a "Jovian-mass exoplanet"
%       with a large surface escape velocity --- can spurt out dust is
%       not straightforward, at least to me.
%       Did you have a specific idea that makes this plausible?
%       Maybe an Io-like moon around the Jupiter, spewing dust?
%       I re-worded to make more neutral but am open to discussion.
%offers a promising origin for
might be related to 
...
%EIC: "their" was ambiguous, so I took a guess. Let me know if I am mistaken.
%their 
the latter's
analysis.
%EIC: "Thus" does not follow from the preceding two sentences, and so
%       was jarring. I just deleted it.
%Thus 
...
%EIC: Personally, I would delete the entire paragraph above as overly
%       speculative. But I understand the need to be politically
%       sensitive.
...
%EIC: "exceed" was ambiguous. Does it imply better or worse?
%       I guessed here that you meant worse, since I can't imagine noise
%       making things better.
%
%       Relatedly, you say "nearly double the resolution" later on in the Conclusion
%       section, which is also technically ambiguous,
%       since double the beam size is actually worse resolution.
%       I'm not sure what you want to do about that, if anything.
%exceed 
be worse than 
...
%EIC: missing \arcsecond but I haven't tex'd to check that my edit compiles.
...
%EIC: The following sentence, and the related Figure 8 (which is a nice figure),
%       seem to contradict the idea, voiced repeatedly throughout this paper
%       (starting with the Abstract and Introduction), that short-wavelength
%       disk thicknesses should be "puffed up" by stellar radiation and winds
%       and should therefore be larger than long-wavelength disk thicknesses.
%       This is especially true when I compare the
%       "modeled" optical-to-infrared thicknesses (Metchev+, Krist+)
%       against the present ALMA thickness (Figure 8).
%
%       I'm not sure what you want to say about this, if anything.
%       If I were the referee, I would ask the authors to comment
%       on this apparent contradiction.
%
%       I might reason that the small particles
%       probed by short-wavelength observations experience a higher collision
%       rate than the larger particles probed by long-wavelength observations,
%       and a higher rate of collisions implies greater damping of random velocities
%       and smaller vertical thicknesses. I inserted such a thought below, but
%       feel free to take it out and replace with something better (or just take it out).
...
%EIC: The following paragraph does not mesh with your actual findings in the
%       paragraph above.
%
%       Simplest solution is just to omit it, which is what I have done; the idea
%       of "puffing up" is already in the Introduction. If you wanted to restore
%       the ideas in the following paragraph, then you will have to reconcile
%       it with the paragraph above --- probably by placing it *before* the text above,
%       and adding some reconciliation statements. So the logic of presentation
%       would be: "(1) The expectation is that OIR scale height > mm scale height.
%       (2) We find just the opposite. (3) Here's a possible reason why."
%       I didn't do this because I was lazy, and because I felt the paper already drags.
%
...
%if any such external pertrubers exist, the constraints placed on the dynamical mass become much more complex.
...
%EIC: changed round brackets to square brackets since there is a round bracket
%       inside
...
%EIC: See earlier comment. "nearly double" here means better
%       resolution, but technically it could mean worse.
%       I left as is, and defer to you.
...
%EIC: following phrase confused me.
%       "eccentricity or non-coplanarity of the feature" --- what feature?
%       The interior ring? The main ring?
%       The sentence as a whole seems too unwieldy, with double and maybe even
%       triple negatives ("but ... minimize ... lost"). Part of the problem is that I'm
%       not used to thinking of bad fits as "losing information". I suggest
%       re-writing the sentence from scratch.
...
%EIC: edited (folded the last sentence of the paragraph into
