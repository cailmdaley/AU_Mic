...
def macgregor_surface_density(n_annuli):
    ...
    #try 2.7
    m_dust_grain = (np.pi * (1.4*u.mm)**2 
        / ( 2.3*u.cm**2/u.g ) ).to_value('g')
    # m_dust_grain = (4/3*np.pi * (1.4*u.mm)**3 * 2*u.g/u.cm**3).to_value('g')
...
sigma[0,1,1].n-sigma[1,0,1].std_dev
sigma[0,1]
...
plt.legend(loc=4); sns.despine(); 
plt.savefig('figures/surface_density', dpi=1000)
plt.show()
# sigma.shape
# sigma[:,:,outer[0][0]-1]
