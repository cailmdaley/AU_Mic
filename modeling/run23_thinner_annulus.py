...
run_name='run23'
def main():
    parser = argparse.ArgumentParser(formatter_class = argparse.RawTextHelpFormatter, description= '''Python commands associated with emcee run21, which has 50 walkers and varies the following parameters:
    1)  disk mass
    ...
    12) annulus mass radius
This run adds a additional annulus of dust to the system, varying the annulus inner radius and mass; the annulus width is set to 1.5 au.''')
...
def make_best_fits(run, concise=False):
    ...
    print(model_params.round(3).to_string())
    print('')
