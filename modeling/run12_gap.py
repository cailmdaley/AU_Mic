...
def main():
    parser = argparse.ArgumentParser(formatter_class = argparse.RawTextHelpFormatter, description= '''Python commands associated with emcee run12, which has 50 walkers and varies the following parameters:
    1)  disk mass
    ...
    11)  august starflux
    12) june starflux
Let's see what a gap does to the inner radius bimodality!''')
    ...
    if args.run:
        mcmc.run_emcee(run_name='run12', nsteps=10000, nwalkers=50, 
            ...
            ('gap_r_in',          0.9,          .2,        (0,       np.inf)),
            ('gap_d_r',           0.4,          .2,        (0,       np.inf)),
            ('inc',               88.5,         0.5,       (0,       90.)),
            ('pa',                128.48,       0.1,       (1,       360)),
        ...
        run = mcmc.MCMCrun('run12', nwalkers=50, burn_in=args.burn_in)
