...
def main():
    ...
    description= '''Python commands associated with emcee run24, which has 50 walkers and varies the following parameters:
    1)  disk mass
    ...
    8)  august starflux
    9)  june starflux
This run is like run 17, but the grid resolution is actually consistent with our fiducial model.
to fix resolution issues, imsize: 512->1024, radiative transfer zrange: 20->5, and most importantly nphi 131-->351 (this corresponds to ~7 grid elements per beam at a radius of 40 au)''')
    ...
    if args.run:
        mcmc.run_emcee(run_name=run_name, nsteps=10000, nwalkers=50,
            ...
            ('m_disk',            -7.54,        0.006,     (-np.inf, np.inf)),
            ('sb_law',            1.,           0.5,       (-5.,     10.)),
            ('r_in',              15,           10,        (0,       np.inf)),
            ('d_r',               28.5,         10,        (0,       np.inf)),
            ('inc',               89,           1,         (0,       90)),
            ('pa',                128.48,       0.1,       (1,       360)),
            ('mar_starflux',      3.9e-4,       2e-5,      (0,       np.inf)),
            ('aug_starflux',      1.5e-4,       2e-5,      (0,       np.inf)),
            ('jun_starflux',      2.3e-4,       2e-5,      (0,       np.inf))])
    else:
