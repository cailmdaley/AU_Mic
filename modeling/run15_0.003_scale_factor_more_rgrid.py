...
def main():
    ...
    description= '''Python commands associated with emcee run15, which has 50 walkers and varies the following parameters:
    1)  disk mass
...
This run is identical to run 10 (scale_factor=0.003) except the number of r_grid points has been doubled to 1000.''')
    ...
    if args.run:
        mcmc.run_emcee(run_name='run15', nsteps=10000, nwalkers=50,
        ...
        run = mcmc.MCMCrun('run15', nwalkers=50, burn_in=args.burn_in)
