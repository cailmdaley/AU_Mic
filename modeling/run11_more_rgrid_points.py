...
def main():
    parser = argparse.ArgumentParser(formatter_class = argparse.RawTextHelpFormatter, description= '''Python commands associated with emcee run11, which has 50 walkers and varies the following parameters:
    1)  disk mass
    ...
    9)  august starflux
    10) june starflux
This run is identical to run 6, except that that r_gridsize has been doubled to investigate how grid resolution affects the solution for r_in preferred by emcee.''')
    ...
    if args.run:
        mcmc.run_emcee(run_name='run11', nsteps=10000, nwalkers=50, 
