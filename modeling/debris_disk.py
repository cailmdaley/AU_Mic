...
class Disk:
    ...
    def __init__(self,params=[-0.5,0.09,1.,10.,1000.,150.,51.5,2.3,1e-4,0.01,33.9,19.,69.3,
    	         ...
    	         rtg=True,vcs=True,line='co',ring=None, annulus=None):
    ...
    def set_params(self,params):
        ...
        if self.annulus is not None:
            self.annulus_Rin = self.ring[0]*Disk.AU # location of the dust annulus
            self.annulus_Rout = self.ring[1]*Disk.AU #width of annulus
            self.annulus_mass = self.ring[2]*Disk.Mearth #total added mass
    ...
    def set_structure(self,exp_temp="False"):
        ...
        rmin = self.Rin if annulus is None \
            else np.min([self.Rin, self.annulus_Rin]) # - minimum r [AU]
        rmax = self.Rout if annulus is None  \
        ...
        if self.annulus is not None:
            w = np.where((rcf > self.annulus_Rin) & (rcf < self.annulus_Rout))
            annulus_sigma = annulus_mass / (np.pi * (annulus_Rout**2 - annulus_Rin**2))
            siggas[w]   += annulus_sigma
    ...
    def add_new_ring(self,Rin,Rout,mass):
        '''Add a ring of dust with a specified inner radius, outer radius, dust-to-gas ratio (defined at the midpoint) and slope of the dust-to-gas-ratio'''
        ...
        w = (self.r>(Rin*Disk.AU)) & (self.r<(Rout*Disk.AU))
        annular_area = np.pi * (Rout**2 - Rin**2)
        print(annular_area)
        self.dtg[w] += dtg*(self.r[w]/Rmid)**(-ppD)
        self.rhoD = self.rhoH2*self.dtg*2*Disk.mh
