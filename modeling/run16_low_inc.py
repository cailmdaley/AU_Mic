...
def main():
    parser = argparse.ArgumentParser(formatter_class = argparse.RawTextHelpFormatter, description= '''Python commands associated with emcee run16, which has 50 walkers and varies the following parameters:
    1)  disk mass
    ...
    8)  august starflux
    9) june starflux
This run fixes the inclination to a value of 85 degrees. Otherwise, it is identical to run11.''')
    ...
    if args.run:
        mcmc.run_emcee(run_name='run16', nsteps=10000, nwalkers=50, 
        ...
        run = mcmc.MCMCrun('run16', nwalkers=50, burn_in=args.burn_in)
