...
run_name='run29'
def main():
    parser = argparse.ArgumentParser(formatter_class = argparse.RawTextHelpFormatter, description= '''Python commands associated with emcee run29, which has 50 walkers and varies the following parameters:
    1)  disk mass
    ...
    9)  august starflux
    10) june starflux
    11) annulus inner radius
    12) annulus outer radius (really inner radius + dr)
    13) annulus mass
This run is a redo of the previous fiducial model run25, but with a broader range of inital positions. This run uses the new Gaia distance to AU Mic and a new gridding resolution.''')
    ...
    if args.run:
        mcmc.run_emcee(run_name=run_name, nsteps=10000, nwalkers=50,
            ...
            ('annulus_r_in',      10.0,         2,         (0,       np.inf)),
            ('annulus_d_r',       4.0,          2,         (0,       np.inf)),
...
param_dict = OrderedDict([
    ...
    ('annulus_r_in',      10.0),
    ('annulus_d_r',       2), # annulus_r_in gets added to this later
    ('annulus_mass',      7.0e-5)])
...
def lnprob(theta, run_name, to_vary):
    ...
    param_dict_copy['annulus_d_r'] += param_dict_copy['annulus_r_in']

    disk_params    = param_dict_copy.values()[:-6]
    starfluxes     = param_dict_copy.values()[-6:-3]
    annulus_params = param_dict_copy.values()[-3:]
...
def make_best_fits(run, concise=False):
    ...
    param_dict['annulus_d_r'] += param_dict['annulus_r_in']
    ...
    if concise:
        ...
        # plotter.save('../writing/figures/annulus_best_fit.pdf', dpi=1000)
        # plotter.show()
