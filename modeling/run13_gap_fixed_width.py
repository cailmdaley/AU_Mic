...
def main():
    parser = argparse.ArgumentParser(formatter_class = argparse.RawTextHelpFormatter, description= '''Python commands associated with emcee run13, which has 50 walkers and varies the following parameters:
    1)  disk mass
    2)  surface brightness power law exponent
    3)  scale factor, multiplied by radius to get scale height
    4)  disk inner radius
    5)  disk outer radius (really inner radius + dr)
    6)  inclination
    7)  position angle
    8)  gap inner radius
    9)  march starflux
    10)  august starflux
    11) june starflux
This run builds upon run 12, fixing the gap width at 0.03 au and doubling the number of radial grid points.''')
    ...
    if args.run:
        mcmc.run_emcee(run_name='run13', nsteps=10000, nwalkers=50, 
        ...
        run = mcmc.MCMCrun('run13', nwalkers=50, burn_in=args.burn_in)
...
def make_fits(model, disk_params):
    ...
    gap_r_in, gap_d_r = disk_params[-3:-1]
    PA = disk_params[-1]

    model_disk = debris_disk.Disk(structure_params, obs=[300, 131, 300, 20])
    model_disk.add_dust_gap(gap_r_in, gap_r_in + gap_d_r)
