...
def model_convolve(im, modelname, filename, coord, ra, dec):
    ...
    im.writeFits('{}.fits'.format(modelname), dpc=8.9, coord='{}'.format(coord))
    ...
    model.close
    subprocess.call(['fits', 'in={}.fits'.format(modelname), 'op=xyin', 'out={}.im'.format(modelname)])
    subprocess.call(['uvmodel', 'model={}.im'.format(modelname), 'vis={}.vis'.format(filename), 'options=replace', 'out={}.vis'.format(modelname)])
    subprocess.call(['fits', 'in={}.vis'.format(modelname), 'op=uvout', 'out={}.uvf'.format(modelname)])
...
def vis_cut(vis, cut, suff, filenames):
    ...
    subprocess.call(['uvaver vis={}.vis select={} out={}.vis'.format(vis, cut, vis+suff)], shell=True)
    filenames.append(vis+suff)

def create_uvf(filename):
    subprocess.call('rm -r {}.uvf'.format(filename), shell=True)
    subprocess.call('fits in={}.vis op=uvout out={}.uvf'.format(filename, filename), shell=True)

def image_vis(vis, pixsize, show=True):
    subprocess.call(['rm -r '+vis+'.{mp,bm,cl,cm}'], shell=True)
    subprocess.call('invert vis={}.vis map={}.mp beam={}.bm cell={} imsize=512 options=systemp,mfs robust=2'.format(vis, vis, vis, pixsize), shell=True)
    subprocess.call(['imstat', 'in={}.mp'.format(vis), "region='boxes(256,0,512,200)'"])
    cutoff = input('Please enter CLEAN cutoff:  ')
    subprocess.call('clean map={}.mp beam={}.bm out={}.cl niters=10000 cutoff={}'.format(vis, vis, vis, cutoff), shell=True) #cutoff=6.290E-05
    subprocess.call('restor map={}.mp beam={}.bm model={}.cl out={}.cm'.format(vis, vis, vis, vis), shell=True)
    if show==True:
